...
export default class WinterJokes extends Component {
  ...
  render() {
    ...
    return (
      <View >
        <Text style={{color: 'blue',marginRight: 10, marginLeft: 10, fontSize: 40}} onPress={answered ? this.nextJoke : this.answer}>{joke.q}</Text>
        {answered && <Text style={{marginRight: 10, marginLeft: 10, fontSize: 50}} onPress={answered ? this.nextJoke : this.answer}>{joke.a}</Text>}
      </View>
    )
  }
}
